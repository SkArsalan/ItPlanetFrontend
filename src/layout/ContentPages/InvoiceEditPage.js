...
function InvoiceEditPage() {
  ...
  useEffect(() => {
    const fetchPurchaseDetails = async () => {
      try {
        const response = await API.get(`/invoice-details/${id}`);
        console.log(response)
        const { customer_name, mobile_number, invoice_number, date, products, total, paid, created_by } = response.data;
